				   ...
				   std::vector<geometry_msgs::PoseArray>& paths)
{
  ...
  for(int i=0; i < (int)source_indices.size(); i++) local_source_indices.push_back(source_indices[i]);
...
bool HeatSurfacePlanner::getCellCentroidData(const shape_msgs::Mesh& mesh, const int id, Eigen::Vector3d& center, double& area)
{
