...
protected:
   ...
   *  bool tool_path_validities        these indicate which if any of the paths provided are invalid. Not sure why an invalid path is returned?
   **/
	...
	for(size_t jj =0; jj<result.tool_raster_paths.size(); jj++)
	  {
	    ...
	    for(size_t kk=0; kk<result.tool_raster_paths[jj].paths.size(); kk++)
	      {
		ROS_INFO("Mesh %ld path %ld has %ld waypoints", jj, kk, result.tool_raster_paths[jj].paths[kk].poses.size());
	      }
	  }
