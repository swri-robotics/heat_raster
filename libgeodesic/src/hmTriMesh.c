...
void hmTriMeshCopy(hmTriMesh* mesh1, const hmTriMesh* mesh2)
{
  ...
  hmTriMeshCopyData(mesh1, mesh2->nVertices, mesh2->vertices, mesh2->nFaces, mesh2->faces);

  mesh1->texCoords = (double*)malloc(mesh2->nVertices * 2 * sizeof(double));
...
void hmTriMeshCopyData(hmTriMesh* mesh, size_t nVertices, const double* vertices, size_t nFaces, const size_t* faces)
{
  ...
  mesh->vertices = (double*)malloc(nVertices * 3 * sizeof(double));
  memcpy(mesh->vertices, vertices, nVertices * 3 * sizeof(double));

  mesh->nFaces = nFaces;
  mesh->faces = (size_t*)malloc(nFaces * 3 * sizeof(size_t));
  memcpy(mesh->faces, faces, nFaces * 3 * sizeof(size_t));

  mesh->referenced = 0;

  /* initialize texture coords to zero */
  mesh->texCoords = (double*)malloc(nVertices * 2 * sizeof(double));
...
void hmTriMeshFindOneRings(hmTriMesh* mesh)
{
  ...
  neighbors = (hmVectorPairSizeTDouble*)malloc(nVertices * sizeof(hmVectorPairSizeTDouble));
  ...
  hmDestroy(mesh->vertexNeighbors);
  mesh->vertexNeighbors = (hmVectorPairSizeTDouble*)malloc(nVertices * sizeof(hmVectorPairSizeTDouble));
  ...
  hmDestroy(mesh->onBoundary);
  mesh->onBoundary = (char*)malloc(nVertices * sizeof(char));
  ...
  for (f = facesBegin; f != facesEnd; f += 3)
  {
    ...
    neighbor.n = f[1];
    neighbor.x = w[2];
    hmVectorPairSizeTDoublePushBack(&neighbors[f[0]], neighbor);
    neighbor.n = f[2];
    neighbor.x = w[1];
    hmVectorPairSizeTDoublePushBack(&neighbors[f[0]], neighbor);
    ...
    neighbor.n = f[2];
    neighbor.x = w[0];
    hmVectorPairSizeTDoublePushBack(&neighbors[f[1]], neighbor);
    neighbor.n = f[0];
    neighbor.x = w[2];
    hmVectorPairSizeTDoublePushBack(&neighbors[f[1]], neighbor);
    ...
    neighbor.n = f[0];
    neighbor.x = w[1];
    hmVectorPairSizeTDoublePushBack(&neighbors[f[2]], neighbor);
    neighbor.n = f[1];
    neighbor.x = w[0];
    hmVectorPairSizeTDoublePushBack(&neighbors[f[2]], neighbor);
...
void hmTriMeshComputeVertexAreas(hmTriMesh* mesh)
{
  ...
  hmDestroy(mesh->vertexAreas);
  mesh->vertexAreas = (double*)malloc(nVertices * sizeof(double));
...
void hmTriMeshComputeWeights(hmTriMesh* mesh)
{
  ...
  hmDestroy(mesh->weights);
  mesh->weights = (double*)malloc(3 * nFaces * sizeof(double));
  w = mesh->weights;
...
void hmTriMeshCacheEdgeData(hmTriMesh* mesh)
{
  ...
  hmDestroy(mesh->edgeNormals);
  hmDestroy(mesh->weightedEdges);
  mesh->edgeNormals = (double*)malloc(9 * nFaces * sizeof(double));
  mesh->weightedEdges = (double*)malloc(9 * nFaces * sizeof(double));
