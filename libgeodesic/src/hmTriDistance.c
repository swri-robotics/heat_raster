...
#include "libgeodesic/hmConstants.h"
...
#define NOTIFY_ZERO_AREA_TRIANGLES 0 /* when set to 1 prints notification of each zero area triangle */
...
void hmTriDistanceSolveHeatEquation(hmTriDistance* distance)
{
  ...
  {
    hmCholeskyFactorBacksolve(&distance->heatFlowNeumannFactor, &distance->heatNeumann, &distance->isSource);
  }
  if (BC > 0.) /* partial Dirichlet */
  {
    hmCholeskyFactorBacksolve(&distance->heatFlowDirichletFactor, &distance->heatDirichlet, &distance->isSource);
  }
...
void hmTriDistanceComputePotential(hmTriDistance* distance)
{
  ...
  for (i = 0; i < (size_t)nFaces; i++)
  {
    ...
    if (!isinf(rMag))
    {
      ...
      if (isnan(potential[f[0]]) || isnan(potential[f[1]]) || isnan(potential[f[2]]))
      {
        fprintf(stderr, "NaN\n============\n");
...
void hmTriDistanceSolvePoissonEquation(hmTriDistance* distance)
{
  ...
  hmCholeskyFactorBacksolve(&distance->laplacianFactor, &distance->distance, &distance->potential);
...
void hmTriDistanceBuildMatrices(hmTriDistance* distance)
{
  ...
  for (i = 0; i < nVertices; i++)
  {
    ...
    for (currentNeighbor = neighborsBegin; currentNeighbor != neighborsEnd + 1; currentNeighbor++)
    {
      ...
      if (!diagonalSet && (currentNeighbor == neighborsEnd || currentNeighbor->n > i))
      {
        diagonalSet = 1;
      ...
      if (currentNeighbor < neighborsEnd && currentNeighbor->n > i)
      {
        ...
        if (boundaryConditions < 1.) /* partial Neumann */
        {
          if (fabs(time * currentNeighbor->x) < 0.00001)
          {
            heatFlowNeumann->values[nz] = -time * 0.00001;
            //		printf("bad weight nz = %ld\n", nz);
          }
          else
          {
