#include <stdlib.h>
#include "libgeodesic/hmSparseMatrix.h"
...
void hmSparseMatrixInitialize(hmSparseMatrix* matrix, size_t nRows, size_t nColumns, size_t nNonZeros)
{
  int sorted = 1;
  int packed = 1;
  ...
  matrix->data =
      cholmod_l_allocate_sparse(nRows, nColumns, nNonZeros, sorted, packed, stype, CHOLMOD_REAL, &hmCHOLMODCommon);

  matrix->nRows = nRows;
  matrix->nColumns = nColumns;
  matrix->nNonZeros = nNonZeros;
  matrix->values = (double*)matrix->data->x;
  matrix->rowIndices = (long int*)matrix->data->i;
  matrix->columnStart = (long int*)matrix->data->p;
}
...
void hmSparseMatrixDestroy(hmSparseMatrix* matrix)
{
  if (matrix->data != NULL)
  {
    cholmod_l_free_sparse(&matrix->data, &hmCHOLMODCommon);
