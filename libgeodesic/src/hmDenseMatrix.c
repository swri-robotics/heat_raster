#include "libgeodesic/hmDenseMatrix.h"
...
void hmDenseMatrixInitialize(hmDenseMatrix* matrix, size_t nRows, size_t nColumns)
{
  size_t leadingDimension = nRows;

  matrix->data = cholmod_l_allocate_dense(nRows, nColumns, leadingDimension, CHOLMOD_REAL, &hmCHOLMODCommon);

  matrix->nRows = nRows;
  matrix->nColumns = nColumns;
  matrix->values = (double*)matrix->data->x;
}
...
void hmDenseMatrixDestroy(hmDenseMatrix* matrix)
{
  if (matrix->data != NULL)
  {
    cholmod_l_free_dense(&matrix->data, &hmCHOLMODCommon);
...
void hmDenseMatrixCopy(hmDenseMatrix* matrix1, const hmDenseMatrix* matrix2)
{
  ...
  matrix1->values = (double*)matrix1->data->x;
}
