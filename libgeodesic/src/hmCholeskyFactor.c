#include <stdio.h>
#include <string.h>
#include "libgeodesic/hmCholeskyFactor.h"
...
void hmCholeskyFactorInitialize(hmCholeskyFactor* factor) { factor->data = NULL; }
...
void hmCholeskyFactorBacksolve(hmCholeskyFactor* factor, hmDenseMatrix* x, const hmDenseMatrix* b)
{
  ...
  x->data = cholmod_l_solve(CHOLMOD_A, factor->data, b->data, &hmCHOLMODCommon);

  x->nRows = x->data->nrow;
  x->nColumns = x->data->ncol;
  x->values = (double*)x->data->x;
}
...
void hmCholeskyFactorBacksolve(hmCholeskyFactor* factor, hmDenseMatrix* x, const hmDenseMatrix* b)
{
  ...
  ma87_solve(job, nrhs, factor->degree, x->values, factor->order, &factor->keep, &hmHSLMA87control, &hmHSLMA87info);
}
